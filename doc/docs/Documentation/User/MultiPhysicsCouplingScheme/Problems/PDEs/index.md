...
!!! note "Full AMR workflow and reference"
    This page only shows how to attach an already-configured AMR driver to a `Problem`. For the complete step-by-step workflow — building an AMR-ready mesh, sharing it across variables, defining the error estimator — see the [AMR tutorial](../../../../../Started/HowTo/Tutorials/AMR/index.md). For the full reference on error estimators (`ErrorEstimatorType::KELLY`, `ErrorEstimatorType::ZZ`) and AMR drivers (`SingleVariableAMR`, `MultiVariableMaxAMR`), see the [Adaptive Mesh Refinement](../../../AMR/index.md) page of the User Manual.