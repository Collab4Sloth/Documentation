...
`SLOTH` provides two interfaces based on [ExprTk](https://www.partow.net/programming/exprtk/) and [Sympy](https://docs.sympy.org/latest/index.html) to manage physical properties. The basic procedure also includes [instructions for installing `SLOTH` with these dependencies.](exprtk_sympy.md). `ExprTk` is optional, while `Sympy` is mandatory.
